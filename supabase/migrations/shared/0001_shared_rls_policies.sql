...
-- This is a SHARED migration - used by both current and legacy applications
-- DEV-ONLY: allow all operations for anon - Replace with auth-based policies before production
...
-- DEV-ONLY policies - allow all operations for anonymous access (development only!)
...
-- ⚠️ PRODUCTION WARNING: These dev policies must be replaced with auth-based policies
-- Example production policy:
-- create policy users_own_data on personas for all using (auth.uid() = user_id);
