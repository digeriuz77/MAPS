...
import { getLearningModuleById, createUserProgress, getUserProgressForModule } from "@/lib/supabase/queries";
...
export default function MIPracticeSessionPage() {
  ...
  const loadModule = async () => {
    try {
      const moduleData = await getLearningModuleById(moduleId);
      if (!moduleData) {
      ...
      let userProgress = await getUserProgressForModule(userId, moduleId);

      if (!userProgress) {
        userProgress = await createUserProgress(userId, moduleId);
      }
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {showFeedback && selectedChoice && (
            <div className={`card ${
              selectedChoice.isCorrect ? "border-l-4 border-l-green-500" : "border-l-4 border-l-amber-500"
            }`}>
              <div className="flex items-start gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  selectedChoice.isCorrect ? "bg-green-100 text-green-600" : "bg-amber-100 text-amber-600"
                }`}>
                  {selectedChoice.isCorrect ? "✓" : "!"}
                </div>
                <div className="flex-1">
                  <h3 className={`font-semibold mb-2 ${
                    selectedChoice.isCorrect ? "text-green-800" : "text-amber-800"
                  }`}>
                  ...
                  <div className="flex items-center justify-between">
                    <span className={`text-sm font-medium ${
                      selectedChoice.isCorrect ? "text-green-600" : "text-amber-600"
                    }`}>
                ...
                {history.slice(-5).map((item, index) => (
                  <div key={index} className="p-2 bg-gray-50 rounded">
                    ...
                    <p className={`text-xs mt-1 ${
                      item.choice.isCorrect ? "text-green-600" : "text-amber-600"
                    }`}>
