...
# Initialize SmartMemoryManager lazily to avoid circular imports
...
class MemoryScoringService:
    ...
    async def retrieve_relevant_memories(self, context: RetrievalContext) -> List[ScoredMemory]:
        ...
        # 3. Get character knowledge (from database)
        character_memories = await self._get_character_memories(context, context_keywords)
        scored_memories.extend(character_memories)

        # 4. CRITICAL: Get long-term memories from database (SmartMemoryManager)
        long_term_memories = await self._get_long_term_memories(context, context_keywords)
        scored_memories.extend(long_term_memories)
    ...
    async def _get_character_memories(self, context: RetrievalContext,
                              context_keywords: List[str]) -> List[ScoredMemory]:
        """Get and score character knowledge memories"""
        scored_memories = []

        # Get character memories from vector service
        character_memories = await character_vector_service.get_relevant_memories(
        ...
        for memory in character_memories:
            ...
            from datetime import timezone
            created_at = datetime.now(timezone.utc) - timedelta(hours=1)  # Treat as recent but not immediate
    ...
    async def _get_long_term_memories(self, context: RetrievalContext,
                                     context_keywords: List[str]) -> List[ScoredMemory]:
        """
        CRITICAL: Get and score long-term memories from database
        This retrieves persona background knowledge from the long_term_memories table
        """
        scored_memories = []
        ...
        try:
            ...
            # Trust-based importance filtering
            max_importance = self._trust_to_importance_threshold(context.trust_level)
            ...
            ltm_memories = result.data if result.data else []

            logger.info(f"📚 Retrieved {len(ltm_memories)} long-term memories from database " +
                       f"(trust: {context.trust_level}, max_importance: {max_importance})")

            # Score each long-term memory
            for idx, memory in enumerate(ltm_memories):
                memory_text = memory.get('content', '')  # Fixed: column is 'content' not 'memory_text'
                importance = float(memory.get('importance', 0.5))  # Fixed: column is 'importance' not 'importance_score'
                ...
                if isinstance(created_at_raw, str):
                    try:
                        ...
                        created_at = datetime.now(timezone.utc) - timedelta(days=30)  # Fallback for old memories
                elif isinstance(created_at_raw, datetime):
                    # If already datetime but timezone-naive, make it aware
                ...
                logger.debug(f"Memory {idx}: created_at type={type(created_at)}, tzinfo={created_at.tzinfo}")
                ...
                try:
                    ...
                    logger.error(f"Failed to score memory '{memory_text[:50]}...': {scoring_error}")
                    logger.error(f"created_at type: {type(created_at)}, tzinfo: {created_at.tzinfo if isinstance(created_at, datetime) else 'N/A'}")
                    raise  # Re-raise to be caught by outer try-except

                # Include all long-term memories (they're already filtered by trust level)
                if final_score > 0.05:  # Very low threshold since they're trust-filtered
                    scored_memories.append(ScoredMemory(
                        memory_id=str(memory.get('id', '')),
                        content=memory_text,
                        memory_type='long_term',
                        relevance_score=self.calculate_relevance_score(memory_text, context.user_input, context_keywords),
                        recency_score=self.calculate_recency_score(created_at),
            ...
            if scored_memories:
                logger.info(f"✅ Scored {len(scored_memories)} long-term memories " +
                           f"(avg importance: {sum(m.importance_score for m in scored_memories)/len(scored_memories):.2f})")

        except Exception as e:
            import traceback
            logger.error(f"Failed to retrieve long-term memories: {e}")
            logger.error(f"Traceback: {traceback.format_exc()}")
            # Don't fail the whole request if long-term memory retrieval fails
            pass

        return scored_memories

    def _trust_to_stage(self, trust_level: float) -> str:
        """Convert numeric trust level to stage name"""
        if trust_level < 0.4:
            return "defensive"
    ...
    def _trust_to_importance_threshold(self, trust_level: float) -> float:
        """
        Convert trust level to maximum importance threshold for memory retrieval
        Lower trust = only show low-importance (surface-level) memories
        Higher trust = show high-importance (deeper) memories
        """
        if trust_level < 0.4:  # defensive
            return 0.40  # Only very surface-level memories
        elif trust_level < 0.6:  # cautious
            return 0.70  # Medium depth memories
        elif trust_level < 0.8:  # opening
            return 0.85  # Deeper memories
        else:  # trusting
            return 1.0  # All memories
